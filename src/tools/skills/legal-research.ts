...
export function createLegalResearchHandler(lawApiOc: string) {
  const rp = (p: LegalParams, name: string, action: string): SkillResult | null =>
    requireParam(p as Record<string, unknown>, name, action);

  return createDispatcher<LegalParams>("legal_research", {

    search_laws: async (p) => {
      const err = rp(p, "query", "search_laws");
      if (err) return err;
      try {
    ...
    get_law_detail: async (p) => {
      const err = rp(p, "law_id", "get_law_detail");
      if (err) return err;
      try {
        const d = await getLawDetail(lawApiOc, p.law_id!);
    ...
    search_admin_rules: async (p) => {
      const err = rp(p, "query", "search_admin_rules");
      if (err) return err;
      try {
    ...
    get_admin_rule_detail: async (p) => {
      const err = rp(p, "admrul_id", "get_admin_rule_detail");
      if (err) return err;
      try {
        const d = await getAdminRuleDetail(lawApiOc, p.admrul_id!);
    ...
    search_ordinances: async (p) => {
      const err = rp(p, "query", "search_ordinances");
      if (err) return err;
      try {
    ...
    get_ordinance_detail: async (p) => {
      const err = rp(p, "ordin_id", "get_ordinance_detail");
      if (err) return err;
      try {
        const d = await getOrdinanceDetail(lawApiOc, p.ordin_id!);
    ...
    search_treaties: async (p) => {
      const err = rp(p, "query", "search_treaties");
      if (err) return err;
      try {
    ...
    get_treaty_detail: async (p) => {
      const err = rp(p, "treaty_id", "get_treaty_detail");
      if (err) return err;
      try {
        const d = await getTreatyDetail(lawApiOc, p.treaty_id!);
    ...
    search_legal_terms: async (p) => {
      const err = rp(p, "query", "search_legal_terms");
      if (err) return err;
      try {
    ...
    get_legal_term_detail: async (p) => {
      const err = rp(p, "term_id", "get_legal_term_detail");
      if (err) return err;
      try {
        const d = await getLegalTermDetail(lawApiOc, p.term_id!);
    ...
    search_english_laws: async (p) => {
      const err = rp(p, "query", "search_english_laws");
      if (err) return err;
      try {
    ...
    get_english_law_detail: async (p) => {
      const err = rp(p, "law_id", "get_english_law_detail");
      if (err) return err;
      try {
        const d = await getEnglishLawDetail(lawApiOc, p.law_id!);
    ...
    search_attached_forms: async (p) => {
      const err = rp(p, "query", "search_attached_forms");
      if (err) return err;
      try {
    ...
    search_law_abbreviations: async (p) => {
      try {
    ...
    get_law_article_sub: async (p) => {
      const err1 = rp(p, "law_id", "get_law_article_sub");
      if (err1) return err1;
      const err2 = rp(p, "article", "get_law_article_sub");
      if (err2) return err2;
      try {
    ...
    search_ai_legal_terms: async (p) => {
      const err = rp(p, "query", "search_ai_legal_terms");
      if (err) return err;
      try {
    ...
    search_linked_ordinances: async (p) => {
      const err = rp(p, "query", "search_linked_ordinances");
      if (err) return err;
      try {
